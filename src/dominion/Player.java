...
public class Player {
	...
	//M�thode dont j'ai besoin pour classe Militia A VOIR
	...
	public CardList totalCards() {
		...
		for(int i=0 ; i<this.discard.size(); i++) {
			toutesCartes.add(this.discard.get(i)); // C'�tait un copi� coll� de la ligne en dessous du premier for, mais il manquait la modif ^^'
		}
	 ...
	 * fait rien.
	 */
	public void playCard(String cardName) {  // A VOIR peut on avoir deux carte avec le m�me nom? J'ai exit au cas ou... TU AS BIEN FAIT JE CROIS
		for(int i =0; i < this.hand.size(); i++) {
	...
	public Card gain(String cardName) {
		...
		this.gain(c); // Je l'ai rajout�, il manquait le placement de la carte dans la d�fausse
	...
	public void endTurn() {
		...
		for(i=0; i<5; i++) { //Il vaudrait peut-�tre mieux utiliser la m�thode drawCard() non ? Elle prend en compte si la pioche est vide...

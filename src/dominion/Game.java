...
public class Game {
	...
	public Game(String[] playerNames, List<CardList> kingdomStacks) {
		if(playerNames == null || kingdomStacks == null) {
			System.err.println("\nles param�tres du constructeur de Game ne peuvent pas �tre null, veuillez les modifier\n");
			System.exit(1);
		}
		...
		// Permet de connaitre l'emplacement de la pile province, pour la m�thode isFinished()
		...
		//Cr�e les instances joueurs et remplit le tableau de joueurs
	...
	public CardList availableSupplyCards() {
		...
		//Si la pile i de la r�serve n'est pas vide, la premi�re carte de cette pile (non nulle) est ajout�e � cartesReserve
		for(int i =0; i < this.supplyStacks.size(); i++) {
		...
		if(cardName != null) {
			for(int i =0; i< this.supplyStacks.size(); i++) {
				if(!this.supplyStacks.get(i).isEmpty() && this.supplyStacks.get(i).get(0) != null && this.supplyStacks.get(i).get(0).getName().equalsIgnoreCase(cardName)) {
					//Retourne la premi�re carte de la pile de la r�serve dont les cartes ont pour nom cardName
	...
	public Card removeFromSupply(String cardName) {
		if(cardName != null) {
			for(int i =0; i< this.supplyStacks.size(); i++) {
				if(!this.supplyStacks.get(i).isEmpty() && this.supplyStacks.get(i).get(0) != null && this.supplyStacks.get(i).get(0).getName().equalsIgnoreCase(cardName)) {
					//Supprime et retourne la premi�re carte de la pile de la r�serve dont les cartes ont pour nom cardName
	...
	//M�thode pour �carter une carte en l'ajoutant � la pile trashedCards du jeu
	...
	public boolean isFinished() {
		//Si la pile Province est vide ou si le nombre de piles disponibles (non vides) est inf�rieur ou �gal au nombre de piles total -3, retourne vrai

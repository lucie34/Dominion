...
public class Mine extends ActionCard {
	...
	public void play(Player p) {
		...
		/*CardList carteTrÈsor = p.getTreasureCards();
		int indice = -1;
		int i =0;
		while(indice == -1) {
			if(carteTrÈsor.get(i).getName().equalsIgnoreCase("Copper")) {
				indice = i;
			}
			else if(carteTrÈsor.get(i).getName().equalsIgnoreCase("Silver")) {
				indice = i;
			}
			i++;
		}
		if(indice != -1) {
			...
			if(p.getGame().availableSupplyCards().contains(or) && carteTrÈsor.get(i).getName().equalsIgnoreCase("Silver")) {
				Card c = p.getGame().getFromSupply("Gold");
				p.incrementHand(c); 
				p.getGame().removeFromSupply("Gold");
				p.removeFromHand(carteTrÈsor.get(indice));
			}
			else if(p.getGame().availableSupplyCards().contains(argent) && carteTrÈsor.get(i).getName().equalsIgnoreCase("Copper")) {
				Card c = p.getGame().getFromSupply("Silver");
				p.incrementHand(c);; 
				p.getGame().removeFromSupply("Silver");
				p.removeFromHand(carteTrÈsor.get(indice));
			}
		}	// si il n'y a plus de piËces or ou argent dans la reserve on ne fait rien*/
	}
}
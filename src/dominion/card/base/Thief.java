...
public class Thief extends AttackCard {
	...
	public boolean devoiler(Player p, CardList pile) {
		...
		String instruction  = "Le joueur actif "+joueurActif.getName()+" peut dÈcider que "+p.getName()+" Ècarte cette carte trÈsor. "+joueurActif.getName()+" voulez-vous faire Ècarter cette carte ? (y/n)";
			...
			if(listeCartesDevoile.get(i) != null && listeCartesDevoile.get(i).getTypes().get(0).equals(CardType.Treasure)) {
				...
				if(nbCartesTresorEcarte < 1) {
					...
					if(rep.equalsIgnoreCase("y")) {
						...
						String question = "Joueur actif "+joueurActif.getName()+" :  voulez-vous rÈcupÈrer cette carte tresor "+listeCartesDevoile.get(i).getName()+" ? (y/n)";

		...
		for(int i = 0; i<listeCartesDevoile.size(); i++) {
			...
			//La carte dÈvoilÈe est une carte trÈsor
				...
				if(nbCartesTresorEcarte < 1) {
					...
					instruction  = "Le joueur actif "+joueurActif.getName()+" peut dÈcider que "+p.getName()+" Ècarte cette carte trÈsor. "+joueurActif.getName()+" : voulez-vous faire Ècarter cette carte ? (y/n)";
					...
					if(rep.equalsIgnoreCase("y")) {
						...
						instruction = "Joueur actif "+joueurActif.getName()+" :  voulez-vous rÈcupÈrer cette carte tresor "+listeCartesDevoile.get(i).getName()+" ? (y/n)";
						rep = joueurActif.choose(instruction, listeChoix, false);

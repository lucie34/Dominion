...
public class Witch extends AttackCard {
	...
	public void attaquer(Player p) {
		...
		for(int i=0; i < p.otherPlayers().size(); i++) {
			p.otherPlayers().get(i).gain(malediction.getName());//Non, dans la mÈthode play Áa fait dÈj‡ la boucle ^^ le p en paramËtre c'est dÈj‡ le p.otherPlayers.get(i), y avait pas d'erreur.
		}
	}
